"""Shared FMR geometry, analytic fields, initialization, and regional norms."""
...
from PyPIC3D.solvers.yee.fmr.transfers import interpolate_coarse_to_fine
...
def _coordinate_mesh(grids, locations, level, guard_cells):
    ...
    axes = tuple(
        axis[g:g + cells]
        for axis, cells in zip(axes, level.shape)
    )
...
def _build_vector_region_masks(
    ...
    for level_index, (level, level_data, weights) in enumerate(zip(
...
def _electromagnetic_energy(
    ...
    for E_level, B_level, E_level_weights, B_level_weights in zip(
        E_active,
        B_active,
        E_weights,
        B_weights,
        ...
        electric_energy += sum(
            ...
            for component, weight in zip(E_level, E_level_weights)
        )
        magnetic_energy += sum(
            jnp.sum(weight * component**2)
            for component, weight in zip(B_level, B_level_weights)
        )
...
def _composite_quadrature_weights(static_parameters, dynamic_parameters):
    ...
    parent_E, parent_B, fine_E, fine_B = build_fmr_quadrature_weights(
        parent_level,
        fine_level,
    ...
    return (parent_E, fine_E), (parent_B, fine_B)
...
def _run_problem(
    ...
    interface_maps = dynamic_parameters.fmr.interface.e_coarse_to_fine_maps
    ...
    E_weights, B_weights = _composite_quadrature_weights(
        static_parameters,
        dynamic_parameters,
    )
